...
export default function DocxSearchApp() {
                ...
                terms.forEach((term) => {
                  if (cellValue.toLowerCase().includes(term)) {
                    results.push({
                      ...
                      source: "embedded",
                      matchedTerm: term,
                      surroundingContext: `Row ${rowIndex + 1}, Cell ${
                        cellIndex + 1
                      }`,
  ...
  const parseDocumentContent = (
    xml: string
  ): { content: string[]; pageBreaks: number[] } => {
    const jsonObj = xmljs.xml2js(xml, {
      compact: true,
    }) as xmljs.ElementCompact;
  ...
  const extractCommentData = (
    ...
    const paragraphs = Array.isArray(comment["w:p"])
      ? comment["w:p"]
      : [comment["w:p"]];
    let text = "";
  ...
  const searchDocument = async () => {
    ...
    setDebugInfo("");
    try {
      const { content, comments, pageBreaks, metadata } =
      ...
      const searchInText = (
        ...
        return text.flatMap((paragraph, index) => {
          ...
          const matches = Array.from(paragraph.matchAll(regex), (m) => ({
            ...
            page:
              source === "content"
                ? pageBreaks.filter((pb) => pb <= index).length + 1
                : 1,
            source: source,
            matchedTerm: term,
      ...
      const contentResults = terms.flatMap((term) =>
        searchInText(content, term, "content")
      );
    ...
    <div className="container mx-auto p-4 max-w-3xl">
      ...
      <div className="mb-4">
        ...
        <Button
          ...
          disabled={!file || !searchTerms || isLoading}
        >
          {isLoading ? (
            <>
